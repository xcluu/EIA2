...
namespace nr4 {
    ...
    class Skifahrer implements IntSkifahrer {
    ...
    function animate(): void {
        ...
        //schnee animieren
        for (let i: number = 1; i < snow.length; i++) {
            snow[i].x += 0.5;
            snow[i].y += 2;
            if (snow[i].y > 200) {
                snow[i].y = 0;
            ...
            drawSnow(snow[i].x, snow[i].y, 0.8 + Math.random() * 1.5, crc2d);
        }
        for (let i: number = 1; i < snowSmall.length; i++) {
            snowSmall[i].x += 0.5;
            snowSmall[i].y += 2;
            if (snowSmall[i].y > 200) {
                snowSmall[i].y = 0;
            }
            if (snowSmall[i].x > 800) {
                snowSmall[i].x = 0;
            }
            drawSnow(snowSmall[i].x, snowSmall[i].y, 0.3 + Math.random() * 0.8, crc2d);
        }
